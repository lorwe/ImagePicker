...
open class ImagePickerController: UIViewController {
    ...
    open override func viewDidAppear(_ animated: Bool) {
        ...
        let galleryHeight: CGFloat = UIScreen.main.nativeBounds.height == 960
                ? ImageGalleryView.Dimensions.galleryBarHeight : self.configuration.galleryMinimumHeight
    ...
    open func showGalleryView() {
        galleryView.collectionViewLayout.invalidateLayout()
        UIView.animate(withDuration: 0.3, animations: {
            self.updateGalleryViewFrames(self.self.configuration.galleryMinimumHeight)
            self.galleryView.collectionView.transform = CGAffineTransform.identity
            self.galleryView.collectionView.contentInset = UIEdgeInsets.zero
        })
    }
    ...
    open func expandGalleryView() {
        ...
        UIView.animate(withDuration: 0.3, animations: {
            self.updateGalleryViewFrames(self.configuration.galleryMaximumHeight)

            let scale = (self.configuration.galleryMaximumHeight - ImageGalleryView.Dimensions.galleryBarHeight) / (self.self.configuration.galleryMinimumHeight - ImageGalleryView.Dimensions.galleryBarHeight)
...
extension ImagePickerController: ImageGalleryPanGestureDelegate {
    ...
    func panGestureDidChange(_ translation: CGPoint) {
        ...
        if galleryHeight >= configuration.galleryMaximumHeight { return }
        ...
        } else if galleryHeight >= self.configuration.galleryMinimumHeight {
            let scale = (galleryHeight - ImageGalleryView.Dimensions.galleryBarHeight) / (self.configuration.galleryMinimumHeight - ImageGalleryView.Dimensions.galleryBarHeight)
    ...
    func panGestureDidEnd(_ translation: CGPoint, velocity: CGPoint) {
        guard let initialFrame = initialFrame else { return }
        let galleryHeight = initialFrame.height - translation.y
        if galleryView.frame.height < configuration.galleryMinimumHeight && velocity.y < 0 {
            showGalleryView()
        } else if velocity.y < -GestureConstants.velocity {
            expandGalleryView()
        } else if velocity.y > GestureConstants.velocity || galleryHeight < configuration.galleryMinimumHeight {
